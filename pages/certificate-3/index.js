...
export default function Certificate3() {
    ...
    const type ='certificate-3.php';
    const [certificate3panel, setCertificate3panel] = useState([])
    ...
    useEffect(()=>{
      ...
      userService.getAllItems(type).then((res) => {  
          setCertificate3panel(res.json_data)
      }) 
    ...
    return(
      ...
      <div id="page-content-wrapper" class="container-fluid reset-padding">
          ...
          <div class="site-bannersection">
              <div class="indicator-view fullsize-banner" style={ {backgroundImage:`url(${certificate3panel.banner_section && certificate3panel.banner_section.back_img})`,} } >
                      <div class="container container-70 indicator-content reset-padding">
                          <div class="row content-row reverse-row-mobile">
                            <div class="col-md-7 title-banner text-left align-self-center">
                                <h2>{certificate3panel.banner_section && certificate3panel.banner_section.title}</h2>
                                <p>
                                {certificate3panel.banner_section && certificate3panel.banner_section.desc}
                                </p>
                                  <button type="button" class="btn gsm-bg-white btn-gsm-md mr-4">{certificate3panel.banner_section && certificate3panel.banner_section.button_name}</button>
                           </div>
                            <div class="col-md-5 picture-banner">
                                <img src={certificate3panel.banner_section && certificate3panel.banner_section.banner_img} class="img-fluid rounded-circle"/>
                            </div>
                          </div>
                      </div>
              </div>
          ...
          <section class="site-body-container">
                ...
                <div class="content-spaces">
                      <h2>{certificate3panel.modules && certificate3panel.modules.title}</h2>
                </div>
                <div class="container container-75 reset-padding certificate-cardarea">
                    <div class="row justify-content-center mx-0 mb-5">
                      {certificate3panel.modules && certificate3panel.modules.first_section.map((items, index) =>
                    ...
                    <div class="row justify-content-center reset-margin">
                      {certificate3panel.modules && certificate3panel.modules.second_section.map((items, index) =>
                        <>
                        {index == 1 ?
                ...
                <div id="satisfaction-certificates-box" class="container container-75 reset-padding certificate-satisfaction-card">
                    <h2>{certificate3panel.collaborate_section && certificate3panel.collaborate_section.title}</h2>
                    <div class="row mx-0 mt-5">
                      {certificate3panel.collaborate_section && certificate3panel.collaborate_section.cards.map((items, index) =>
